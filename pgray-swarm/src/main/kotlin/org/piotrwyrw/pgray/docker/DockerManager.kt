...
import org.piotrwyrw.pgray.docker.status.parseHealthStatus
...
class DockerManager {
    ...
    fun pingDockerServer(ok: () -> Unit, error: () -> Unit) {
        try {
            docker.pingCmd().exec()
            ok()
        } catch (t: Throwable) {
    ...
    fun allocNextAvailablePort(): Int {
    ...
    fun createPostgresContainer(): PostgresContainer {
        ...
        val port = allocNextAvailablePort()
